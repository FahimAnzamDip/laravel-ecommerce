                ...
                <div class="col-sm-12">
                    <div class="breadcrumb-content text-center ptb-70">
                        @if(request()->has('category_id'))
                            <h1>{{ \App\Category::find(request()->get('category_id'))->category_name }}</h1>
                        @elseif(request()->has('sub_category_id'))
                            <h1>{{ \App\SubCategory::find(request()->get('sub_category_id'))->sub_category_name }}</h1>
                        @else
                            <h1>ALL PRODUCTS</h1>
                        @endif
                        <ul class="breadcrumb-list breadcrumb">
                            @if(request()->has('category_id'))
                                ...
                                <li>{{ \App\Category::find(request()->get('category_id'))->category_name }}</li>
                            @elseif(request()->has('sub_category_id'))
                                <li><a href="{{ route('home.page') }}">home</a></li>
                                <li><a href="{{ route('shop.page') }}">Shop</a></li>
                                <li>
                                    <a href="{{ route('shop.page', ['category_id' => \App\SubCategory::find(request()->get('sub_category_id'))->category->id]) }}">
                                        {{ \App\SubCategory::find(request()->get('sub_category_id'))->category->category_name }}
                                    </a>
                                </li>
                                <li>{{ \App\SubCategory::find(request()->get('sub_category_id'))->sub_category_name }}</li>
                            @else
                                <li><a href="{{ route('home.page') }}">home</a></li>
                                <li>Shop</li>
                            @endif
            ...
            <div class="row">
                ...
                <div class="col-md-9 col-md-push-3">
                    ...
                    <div class="sidebar-desc-content">
                        @if(request()->has('category_id'))
                            <p>You are viewing <strong>Category: {{ \App\Category::find(request()->get('category_id'))->category_name }}</strong> products</p>
                        @elseif(request()->has('sub_category_id'))
                            <p>You are viewing <strong>Sub Category: {{ \App\SubCategory::find(request()->get('sub_category_id'))->sub_category_name }}</strong> products</p>
                        @else
                            <p>You are viewing all the products</p>
                        @endif
                        <hr>
                    </div>
                    <!-- Sidebar Right Top Content Start -->
                    ...
                    <aside class="categorie-sidebar mb-100">
                        ...
                        <div class="categorie-module mb-80">
                            ...
                            <ul class="categorie-list">
                                <li><a href="{{ route('shop.page') }}">All Products ({{ \App\Product::count() }})</a></li>
                                @foreach(\App\Category::all() as $category)
                                    <li>
                                        <a href="{{ route('shop.page', ['category_id' => $category->id]) }}">{{ $category->category_name }} ({{ \App\Product::where('category_id', $category->id)->count() }})</a>
                                        @if($category->subCategories->isNotEmpty())
                                            <ul class="sub-categorie pl-30">
                                                @foreach($category->subCategories as $sub_category)
                                                    <li>
                                                        <a href="{{ route('shop.page', ['sub_category_id' => $sub_category->id]) }}">{{ $sub_category->sub_category_name }} ({{ \App\Product::where('sub_category_id', $sub_category->id)->count() }})</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
